...
export function registerLogTools(server, auth) {
    // --- List Logs ---
    server.tool("list_logs", `List and filter LLM request logs via GET /api/request-logs/.
...
EXAMPLE FILTERS:
{
  ...
  "metadata__session_id": {"operator": "", "value": ["abc123"]}
        ...
        const queryParams = { page_size: limit, page, sort_by };
        if (start_time)
    ...
    // --- Get Single Log ---
    server.tool("get_log_detail", `Retrieve complete details of a single log via GET /api/request-logs/{id}/.
